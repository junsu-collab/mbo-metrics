import * as XLSX from "xlsx";

import { maxWS, memberSlots, memberTotal, taskValues } from "@/lib/calc";
import type { MemberData, Settings } from "@/types";

/** 연도 단위 전체 팀원 종합 순위 엑셀 */
export function exportAllExcel(
  members: Record<string, MemberData>,
  settings: Settings,
  year: string
): boolean {
  const names = Object.keys(members);
  if (!names.length) return false;

  const ranked = names
    .map((n) => ({ n, tasks: members[n].tasks.length, total: memberTotal(members[n], settings) }))
    .sort((a, b) => b.total - a.total);

  const head = ["순위", "팀원", "업무 수", "최종 총점"];
  const body = ranked.map((m, i) => [i + 1, m.n, m.tasks, +m.total.toFixed(2)]);
  const title = `${year}년 뉴스디자인팀 업무평가 종합순위`;
  const aoa = [
    [title],
    [`총 ${ranked.length}명 · 생성 ${new Date().toLocaleDateString("ko-KR")}`],
    [],
    head,
    ...body,
  ];

  const ws = XLSX.utils.aoa_to_sheet(aoa);
  ws["!cols"] = [{ wch: 6 }, { wch: 16 }, { wch: 8 }, { wch: 12 }];
  ws["!merges"] = [
    { s: { r: 0, c: 0 }, e: { r: 0, c: 3 } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: 3 } },
  ];
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, `${year} 종합순위`);
  XLSX.writeFile(wb, `MBO_${year}_종합순위.xlsx`);
  return true;
}

/** 팀원 1명의 업무별 상세 점수 엑셀 */
export function exportMemberExcel(name: string, member: MemberData, settings: Settings): boolean {
  if (!member.tasks.length) return false;
  const ceil = maxWS(settings);
  const head = [
    "팀원",
    "MBO 항목",
    "항목 배점",
    "업무종류",
    "비중%",
    "난이도",
    "난이도계수",
    "주간보고",
    "기여계수",
    "업무 W",
    "항목 팀장S",
    "항목 팀원S",
    "항목 S환산",
    "항목 W합계",
    "기준 상한",
    "항목 비율%",
    "항목 점수",
  ];
  const rows: (string | number)[][] = [];
  memberSlots(member, settings)
    .filter((r) => r.n > 0)
    .forEach((r) => {
      const ss = member.categoryScores?.[r.mbo.id] ?? { leader: null, member: null };
      r.tasks.forEach((t, i) => {
        const { dif, rep } = taskValues(t, settings);
        const k = dif.coef * rep.coef;
        const first = i === 0;
        const wPct = Math.round((r.pRatios[i] || 0) * 100);
        rows.push([
          name,
          first ? r.mbo.label : "",
          first ? r.mbo.pts : "",
          t.taskName,
          wPct,
          dif.label,
          dif.coef,
          rep.label,
          rep.coef,
          +k.toFixed(3),
          first ? ss.leader ?? "미입력" : "",
          first ? ss.member ?? "미입력" : "",
          first ? (r.conv != null ? +r.conv.toFixed(3) : "미평가") : "",
          first ? +r.weightedW.toFixed(3) : "",
          first ? +ceil.toFixed(2) : "",
          first ? (r.status === "ok" ? +(r.ratio * 100).toFixed(1) : "—") : "",
          first ? (r.status === "ok" ? +r.pts.toFixed(2) : 0) : "",
        ]);
      });
    });
  rows.push([]);
  rows.push([
    name,
    "개인 최종 총점(기준 상한 100)",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    +memberTotal(member, settings).toFixed(2),
  ]);

  const ws = XLSX.utils.aoa_to_sheet([head, ...rows]);
  ws["!cols"] = head.map((h, i) => ({
    wch: i === 1 ? 24 : i === 3 ? 26 : i === 4 ? 7 : h.length < 6 ? 8 : 11,
  }));
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, name.slice(0, 28));
  XLSX.writeFile(wb, `MBO_${name}.xlsx`);
  return true;
}
